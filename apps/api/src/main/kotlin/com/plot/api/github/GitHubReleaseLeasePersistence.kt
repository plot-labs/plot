...
class GitHubReleaseLeasePersistence(
	...
	override fun claimNext(workerId: String, now: Instant, leaseTimeout: Duration): GitHubReleaseDraftRequest? {
		...
		val updated = sqlExecutor.update(
			...
			set status = 'RESOLVING', attempt_count = attempt_count + 1,
			    transition_version = transition_version + 1, claimed_by = ?,
			...
			candidate.workspaceId, candidate.id, candidate.transitionVersion,
		)
	...
	override fun renewClaim(requestId: UUID, transitionVersion: Long, workerId: String, now: Instant): Boolean =
		sqlExecutor.update(
			"""
			update github_release_draft_requests set heartbeat_at = ?, updated_at = ?
			where workspace_id = ? and id = ? and transition_version = ? and claimed_by = ?
			  and status in ('RESOLVING', 'GENERATING')
			""".trimIndent(),
			Timestamp.from(now), Timestamp.from(now), requestWorkspaceId(requestId), requestId, transitionVersion, workerId,
		) == 1
	...
	override fun retry(requestId: UUID, workspaceId: UUID, transitionVersion: Long): GitHubReleaseRetryResult {
		...
		val updated = sqlExecutor.update(
			...
			where workspace_id = ? and id = ? and transition_version = ? and status = 'FAILED'
			""".trimIndent(),
			nextAttempt, Timestamp.from(now), Timestamp.from(now), workspaceId, requestId, transitionVersion,
		)
	...
	override fun scheduleRetry(requestId: UUID, transitionVersion: Long, nextAttemptAt: Instant, errorCode: String) {
		...
		val updated = sqlExecutor.update(
			...
			where workspace_id = ? and id = ? and transition_version = ?
			  and status in ('RESOLVING', 'GENERATING')
			""".trimIndent(),
			errorCode, Timestamp.from(nextAttemptAt), Timestamp.from(now),
			requestWorkspaceId(requestId), requestId, transitionVersion,
		)
		requireExactlyOne(updated, "Release request transition was lost")
	}
	...
	override fun fenceSourceScope(workspaceId: UUID, sourceScopeId: UUID, now: Instant, errorCode: String): Int {
		...
		val updated = sqlExecutor.update(
			...
			where workspace_id = ? and source_scope_id = ? and status in ('QUEUED', 'RESOLVING', 'GENERATING')
			""".trimIndent(),
			errorCode, Timestamp.from(now), Timestamp.from(now), workspaceId, sourceScopeId,
		)
		sqlExecutor.query(
	...
	override fun recoverStaleClaims(now: Instant, leaseTimeout: Duration): Int {
		...
		return candidates.sumOf { candidate ->
			...
			val nextAttempt = if (candidate.artifactWorkflowRunId == null && candidate.agentRunId == null) {
				Timestamp.from(now)
			} else null
			val updated = sqlExecutor.update(
				"""
				...
				set status = ?, transition_version = transition_version + 1, claimed_by = null,
				    ...
				    next_attempt_at = case when cast(? as timestamptz) is not null then cast(? as timestamptz) else next_attempt_at end,
				    updated_at = ?
				where workspace_id = ? and id = ? and transition_version = ? and claimed_by is not null
				  and (heartbeat_at is null or heartbeat_at < ?)
				  and status in ('QUEUED', 'RESOLVING', 'GENERATING')
				""".trimIndent(),
				recoveredStatus, nextAttempt, nextAttempt, Timestamp.from(now),
				requestWorkspaceId(candidate.requestId), candidate.requestId, candidate.transitionVersion,
				Timestamp.from(staleBefore),
			)
			when (updated) {
				0 -> 0
	...
	override fun recordReconcileDiagnostic(requestId: UUID, transitionVersion: Long, errorCode: String) {
		...
		val updated = sqlExecutor.update(
			...
			where workspace_id = ? and id = ? and transition_version = ? and status = 'GENERATING'
			""".trimIndent(),
			errorCode, Timestamp.from(clock.instant()), requestWorkspaceId(requestId), requestId, transitionVersion,
		)
		requireExactlyOne(updated, "Release request transition was lost")
	}
