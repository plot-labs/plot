...
class GitHubImportService(
	...
	fun start(scopeId: UUID, request: GitHubImportRequest): GitHubImportResponse {
		...
		val import = reservationService.reserve(scope, request)
		try {
		...
		return sqlExecutor.query(
			...
			{ rs, _ ->
				GitHubImportResponse(
					...
					sourceScopeId = requireNotNull(rs.getObject(2, UUID::class.java)),
					from = requireNotNull(rs.getTimestamp(3)).toInstant(),
					to = requireNotNull(rs.getTimestamp(4)).toInstant(),
					status = requireNotNull(rs.getString(5)),
					...
					startedAt = requireNotNull(rs.getTimestamp(12)).toInstant(),
...
class GitHubImportPersistenceService(
	...
	fun complete(
		...
		val updatedRows = sqlExecutor.update(
			"""
		...
		return GitHubImportResponse(
			...
			errorCode = null,
			errorMessage = null,
			startedAt = sqlExecutor.queryForObject(
