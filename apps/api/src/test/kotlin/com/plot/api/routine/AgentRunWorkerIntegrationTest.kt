	...
	fun isolateScenario() {
		...
		jdbcTemplate.update(
			...
			set status = 'FAILED', failure_code = 'TEST_ISOLATION', claimed_by = null, claimed_at = null,
			    next_attempt_at = null, finished_at = coalesce(finished_at, greatest(now(), created_at)), updated_at = now()
		...
		jdbcTemplate.update(
			...
			set status = 'FAILED', error_code = 'TEST_ISOLATION', claimed_by = null, claimed_at = null,
			    heartbeat_at = null, next_attempt_at = null, finished_at = coalesce(finished_at, greatest(now(), created_at)), updated_at = now()
