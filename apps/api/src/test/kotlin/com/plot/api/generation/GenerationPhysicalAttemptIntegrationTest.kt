...
class GenerationPhysicalAttemptIntegrationTest {
	...
	fun isolateGenerationQueue() {
		jdbcTemplate.update(
			...
			where workspace_id = ? and status in ('QUEUED', 'WRITING', 'REVIEWING', 'REWRITING')
			""".trimIndent(),
	...
	fun transientFailureCreatesANewAttemptForTheSameLogicalStep() {
		...
		val worker = GenerationRunWorker(persistence, workflow, gateway, workerId = "retry-worker")
	...
	fun thirdTransientFailureStopsWithoutAFourthProviderCall() {
		...
		val worker = GenerationRunWorker(persistence, workflow, gateway, workerId = "bounded-worker")
	...
	fun heartbeatFailureBeforeModelResponseDiscardsTheStaleResult() {
		...
		val leaseFactory = GenerationRunLeaseFactory { claim ->
			...
			GenerationRunLeaseHandle(lease) {}
		}
