...
/**
 * Plot mark used as a negative-space mask.
 * @label Logo Image
 */
uniform sampler2D u_logo;

/**
 * Show the Plot mark treatment.
 * @label Show Logo
 * @default 0
 * @range 0, 1
 */
...
/**
 * Darkness behind the Plot mark.
 * @label Logo Halo
 * @default 0.5
 * @range 0, 0.8
 */
uniform float u_logoHalo;

/**
 * Animate the sampled image and dither threshold.
 * @label Motion
 * @default 0
 * @range 0, 1
 */
...
/**
 * Distance the sampled image drifts through the dither grid.
 * @label Motion Amount
 * @default 0.0025
 * @range 0, 0.012
 */
...
/**
 * Amount the dither threshold breathes over time.
 * @label Shimmer
 * @default 0.025
 * @range 0, 0.1
 */
...
vec2 coverUv(vec2 fragPix) {
  ...
  float texAspect = max(u_imageAspect, 0.1);
...
void main() {
  ...
  vec2 sampleUv = coverUv((cellId + 0.5) * cell);
  float motionPhase = u_time * u_motionSpeed;
  vec2 flow = vec2(
    sin(sampleUv.y * 9.0 + motionPhase),
    cos(sampleUv.x * 7.0 - motionPhase * 0.8)
  ) * u_motionAmount * u_motion;
  sampleUv = clamp(sampleUv + flow, 0.0, 1.0);
  vec3 rgb = tone(texture2D(u_image, sampleUv).rgb);
  float lum = dot(rgb, vec3(0.299, 0.587, 0.114));
  float logoWidth = 0.22;
  vec2 logoSize = vec2(logoWidth, logoWidth * u_imageAspect);
  vec2 logoOrigin = vec2(0.67, 0.20);
  vec2 logoUv = (sampleUv - logoOrigin) / logoSize;
  float logoBounds =
    step(0.0, logoUv.x) *
    step(0.0, logoUv.y) *
    step(logoUv.x, 1.0) *
    step(logoUv.y, 1.0);
  float logoMask =
    texture2D(u_logo, clamp(logoUv, 0.0, 1.0)).a *
    logoBounds *
    u_showLogo;
  vec2 haloUv = (sampleUv - (logoOrigin + logoSize * 0.5)) / (logoSize * 0.88);
  float logoHalo =
    (1.0 - smoothstep(0.12, 1.0, length(haloUv))) *
    u_logoHalo *
    u_showLogo;
  lum = mix(lum * (1.0 - logoHalo), 1.0, logoMask);
  ...
  if (ascii) {
    ...
    float shimmer = sin(
      motionPhase * 2.0 + cellId.x * 0.11 + cellId.y * 0.08
    ) * u_shimmer * u_motion;
    float t = clamp(bayer4(cellId) + shimmer, 0.0, 1.0);
    if (u_keepColor == 1) {
      vec3 v = rgb * levels;
