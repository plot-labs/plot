...
export function OnboardingFlow({ embedded = false, initialStep = 1, onComplete, onResultReview }: { embedded?: boolean; initialStep?: Step; onComplete?: () => void; onResultReview?: () => void }) {
  ...
  const [busy, setBusy] = useState<"connect" | "routine" | "run" | null>(null);
  ...
  async function connectGitHub() {
    ...
    try {
      ...
      if (requiresReauth) {
        setNeedsReauth(true);
        setError("GitHub access was revoked or expired. Connect GitHub again to reconnect.");
      } else {
  ...
  async function reauthenticate() {
    ...
    setNeedsReauth(false);
    try {
      const request = await plotApiClient.startGitHubProductOAuth("/chat");
      window.location.assign(request.authorizationUrl);
    } catch (failure) {
      setBusy(null);
      setNeedsReauth(true);
      setError(failure instanceof Error ? failure.message : "GitHub account authorization could not start.");
    }
  }
  ...
  async function runFirstRoutine() {
    ...
    try {
      ...
      setBusy(null);
      setError(failure instanceof Error ? failure.message : "Repository history could not be imported.");
    }
  }
...
function CompletedRoutineStep({ routine, onContinue, onBack, onDismiss }: { routine: Routine; onContinue: () => void; onBack: () => void; onDismiss?: () => void }) {
  return <section className="space-y-6"><header><h1 className="font-display text-[34px] font-normal leading-[1.08] tracking-[-0.03em] sm:text-[38px]">Your first Routine</h1><p className="mt-3 text-[14px] leading-6 text-black/52 dark:text-white/50">Plot will run this Routine whenever the repository publishes a release.</p></header><dl className="border-y border-black/[0.075] text-[13px] dark:border-white/[0.09]"><div className="grid grid-cols-[96px_1fr] gap-4 border-b border-black/[0.06] py-3 dark:border-white/[0.07]"><dt className="text-black/38 dark:text-white/38">Routine</dt><dd className="font-medium text-black/76 dark:text-white/78">{routine.name}</dd></div><div className="grid grid-cols-[96px_1fr] gap-4 border-b border-black/[0.06] py-3 dark:border-white/[0.07]"><dt className="text-black/38 dark:text-white/38">Repository</dt><dd className="text-black/68 dark:text-white/70">{routine.sourceLabel}</dd></div><div className="grid grid-cols-[96px_1fr] gap-4 py-3"><dt className="text-black/38 dark:text-white/38">Trigger</dt><dd className="text-black/68 dark:text-white/70">Release published</dd></div></dl><div className="flex items-center gap-2"><button type="button" onClick={onContinue} className="inline-flex h-10 items-center rounded-full bg-[#252a30] px-4 text-[13px] font-medium text-white transition hover:-translate-y-px hover:bg-[#16191d] active:translate-y-0 dark:bg-white dark:text-[#18191b]">Continue</button><button type="button" onClick={onBack} className="inline-flex h-10 items-center rounded-full px-3.5 text-[13px] text-black/44 transition hover:bg-white/50 hover:text-black/68 dark:text-white/45 dark:hover:bg-white/[0.07] dark:hover:text-white/70">Back</button><Link href="/automation" onClick={onDismiss} className="ml-auto text-[12px] text-black/40 underline-offset-4 hover:underline dark:text-white/40">View Routines</Link></div></section>;
}

function ResultStep({ routine, running, historyImported, error, onRun, onReview, onDismiss }: { routine: Routine | null; running: boolean; historyImported: boolean; error: string | null; onRun: () => void; onReview?: () => void; onDismiss?: () => void }) {
  const artifactId = routine?.latestExecution?.artifactId;
  const title = artifactId ? "Your first result is ready" : running ? "Importing repository history" : historyImported ? "Your Routine is ready" : "Prepare your first Routine";
  const description = artifactId ? "Review the source-backed artifact Plot created." : running ? "Plot is importing up to one year of repository activity so later releases have context." : historyImported ? "Plot will draft when GitHub publishes the next release. A generic Run now is not used for release Routines." : "Import history so the next release has context. Plot waits for a real tag or published release instead of inventing a first changelog.";
  return <section className="pt-5"><header><h1 className="max-w-[480px] text-balance font-display text-[32px] font-normal leading-[1.08] tracking-[-0.035em] sm:text-[36px]">{title}</h1><p className="mt-3 max-w-[500px] text-pretty text-[14px] leading-6 text-black/52 dark:text-white/50">{description}</p></header>{routine && <dl className="mt-7 border-y border-black/[0.075] text-[13px] dark:border-white/[0.09]"><div className="grid grid-cols-[96px_1fr] gap-4 border-b border-black/[0.06] py-3 dark:border-white/[0.07]"><dt className="text-black/38 dark:text-white/38">Routine</dt><dd className="font-medium text-black/76 dark:text-white/78">{routine.name}</dd></div><div className="grid grid-cols-[96px_1fr] gap-4 border-b border-black/[0.06] py-3 dark:border-white/[0.07]"><dt className="text-black/38 dark:text-white/38">Repository</dt><dd className="text-black/68 dark:text-white/70">{routine.sourceLabel}</dd></div><div className="grid grid-cols-[96px_1fr] gap-4 py-3"><dt className="text-black/38 dark:text-white/38">Trigger</dt><dd className="text-black/68 dark:text-white/70">Release published</dd></div></dl>}{error && <p className="mt-5 text-sm text-rose-700 dark:text-rose-300" role="alert">{error}</p>}<div className="mt-7 flex items-center gap-2">{artifactId ? <Link href={`/contents?artifact=${encodeURIComponent(artifactId)}`} onClick={onReview} className="inline-flex h-10 items-center rounded-full bg-[#252a30] px-4 text-[13px] font-medium text-white transition hover:-translate-y-px hover:bg-[#16191d] active:translate-y-0 dark:bg-white dark:text-[#18191b]">Review result</Link> : historyImported ? <Link href="/automation" onClick={onDismiss} className="inline-flex h-10 items-center rounded-full bg-[#252a30] px-4 text-[13px] font-medium text-white transition hover:-translate-y-px hover:bg-[#16191d] active:translate-y-0 dark:bg-white dark:text-[#18191b]">View Routines</Link> : <button type="button" onClick={onRun} disabled={!routine || running} className="inline-flex h-10 items-center gap-2 rounded-full bg-[#252a30] px-4 text-[13px] font-medium text-white transition hover:-translate-y-px hover:bg-[#16191d] active:translate-y-0 disabled:cursor-wait disabled:opacity-45 dark:bg-white dark:text-[#18191b]">{running && <HugeiconsIcon icon={Loading03Icon} size={15} className="animate-spin" strokeWidth={1.6} />}{running ? "Importing…" : "Import history"}</button>}{!historyImported || artifactId ? <Link href="/automation" onClick={onDismiss} className="inline-flex h-10 items-center rounded-full px-3.5 text-[13px] text-black/44 transition hover:bg-white/50 hover:text-black/68 dark:text-white/45 dark:hover:bg-white/[0.07] dark:hover:text-white/70">View Routines</Link> : null}</div></section>;
}
