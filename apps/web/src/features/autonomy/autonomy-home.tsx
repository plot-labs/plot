...
export function AutonomyHomeWorkspace({ view = "overview" }: { view?: "overview" | "activity" }) {
  ...
  return <section className="min-h-[calc(100dvh-49px)] overflow-y-auto bg-[#f8fafc] px-6 pb-16 pt-14 dark:bg-[#18181b] lg:h-full lg:min-h-0 lg:px-12">
    ...
    {data && <>
      ...
      {!loading && data.items.length === 0 && <div className={`${surfaceClass} mt-8 px-5 py-10 text-center text-sm leading-6 text-black/45 dark:text-white/45`}><h2 className="font-medium text-black/65 dark:text-white/65">No assessed changes yet</h2><p className="mt-2 text-sm leading-6 text-black/52 dark:text-white/52">New connected activity will appear here after assessment. Not every change needs a customer update.</p></div>}
