...
export function AutonomyHomeWorkspace() {
    ...
    return <ul className="divide-y divide-black/10 dark:divide-white/10">{items.map((item) => <li key={item.id} className="py-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {item.chatId && item.agentRunId && <Link className={buttonClass} href={`/chat?chat=${encodeURIComponent(item.chatId)}&agent=${encodeURIComponent(item.agentRunId)}`}>Open draft activity</Link>}
  ...
  return <main className="mx-auto w-full max-w-4xl px-5 py-8 sm:px-8">
    <div className="flex items-start justify-between gap-4">
      <div><h1 className="text-2xl font-semibold tracking-tight">Home</h1><p className="mt-2 text-sm text-black/60 dark:text-white/60">Customer updates worth preparing, and changes waiting for more context.</p></div>
    ...
    {data && <>
      ...
      {results.length > 0 && <section className="mt-8" aria-label="Draft activity"><h2 className="font-semibold">Draft activity</h2><p className="mt-1 text-sm text-black/50 dark:text-white/50">Open the activity to inspect evidence and review the draft.</p>{rows(results)}</section>}
      {pending.length > 0 && <section className="mt-8" aria-label="Changes under consideration"><h2 className="font-semibold">Changes under consideration</h2>{rows(pending)}</section>}
      {excluded.length > 0 && <details className="mt-8 border-t border-black/10 pt-4 dark:border-white/10"><summary className="cursor-pointer text-sm font-medium">Excluded and dismissed ({excluded.length})</summary>{rows(excluded)}</details>}
    </>}
  </main>;
}
