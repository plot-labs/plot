...
describe("Autonomy Home", () => {
  ...
  it("shows held evidence separately from draft activity without implying publication", async () => {
    api.getAutonomyHome.mockResolvedValue({ items: [item(), item({ id: "op-2", title: "API changes", disposition: "ELIGIBLE", agentRunId: "agent-2", chatId: "chat-2", missingFacts: [] })] });
  ...
  it("uses the displayed version to dismiss and exposes restore", async () => {
    api.getAutonomyHome.mockResolvedValue({ items: [item()] });
  ...
  it("discards a late response from the previous workspace", async () => {
    ...
    api.getAutonomyHome.mockResolvedValueOnce({ items: [] });
    render(<AutonomyHomeWorkspace view="activity" />);
    ...
    await act(async () => { finishOld({ items: [item()] }); });
...
it("does not apply an old workspace decision after a switch", async () => {
  ...
  api.getAutonomyHome.mockResolvedValueOnce({ items: [item()] });
  api.getAutonomyHome.mockResolvedValueOnce({ items: [item({ title: "New workspace update" })] });
