...
Before adding publish automation, validate the autonomous draft loop with the
first connected shipped-work adapter:
...
4. Ask whether they would keep the agent enabled for next week's release and
   what they would pay to avoid the manual update cycle.

The strongest validation signal is not praise. It is a user asking to connect
their real private source and use Plot on the next release.
