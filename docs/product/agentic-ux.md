...
```txt
Agent workbench:
task prompt -> agent work -> artifacts, tests, diff -> human review

Style-native writing workspace:
writing intent -> agent interview -> styled drafts -> human review

Plot:
shipping intent -> agent work -> source, claim, style artifacts -> human approval
```
...
what the agent did, and approve the artifacts.
...
- The user reviews artifacts instead of reading every tool call.
...
- Use `TaskArtifact` for source maps, claim maps, style reports, draft packs,
...
- Coding-agent products center around isolated task environments, command/test
  results, and review before merge.
...
- Generation controls are `shipping window`, `voice`, `audience`, `channels`,
  and `review strictness`.
- Show multiple update artifacts in one pack: changelog, docs update, customer
  update, launch or social draft.
- Keep source evidence and style match next to the draft, not in a separate
  settings page.
...
- Strong agentic work tools treat agents as part of product work: agents can
  receive work, update activity, produce output, and enter review like humans.
...
- Use a `Sources` timeline for shipped work, but make source
  selection editable before generation and approval.
- Use a dense task detail screen: center work content, activity timeline,
  and a right rail for source scope, voice, checks, reviewer, and approval.
...
- Parallel work needs a manager view with status, attention, and review state.
- Review is a separate surface from chat: diffs/checks/comments in ADEs become
  source/claim/style checks in Plot.
...
- Use `ReviewCheck` or typed `TaskArtifact` entries for source coverage, claim
  evidence, unsupported claims, style match, channel completeness, and approval.
...
- Agentic development environments center on parallel agent work, isolated
  task/workspace state, live execution, review/check surfaces, and
  archive/merge/approval handoff.
...
Packs         approved and draft update packs
...
ready-for-review tasks should appear inside it first. `Autonomous` can become a
...
Review: Source-backed, approval required
```
...
Right rail:
  source scope, voice, pack artifacts, checks, owner, approvals
```
...
Approved     human approved one or more variants
Failed       agent failed with a reviewable error summary
```
...
The review view should be three-pane:

```txt
Source timeline   Draft variants             Review
---------------   --------------             ------
PRs               Changelog                  Claim evidence
Issues            Docs update                Caveats
Commits           Customer update            Style match
Releases          Launch/social              Approval
```

The user can accept, edit, regenerate, or mark a claim as unsupported.
When the agent is uncertain, the middle pane can show multiple writing-style
directions with style and evidence indicators for each.
...
Draft request
  ...
  -> Claim/style verification artifacts
  -> Review checks
  -> Ready for review
  -> Human approval
```
...
- Add `tasks` for durable work visible in the session task/review area.
- Add `task_artifacts` for reviewable outputs: source map, claim map, style
  report, agent plan, verification summary, and content pack links.
- Add `source_selection_items` so the source timeline becomes a
  reviewable selection, not hidden pipeline state.
- Add `review_checks` or typed check artifacts for approval readiness signals.
...
- Keep publishing approval-gated in v0.
- Keep source adapters permissioned and workspace-scoped.
...
- Do not add autonomous publishing before the review loop is trusted.
